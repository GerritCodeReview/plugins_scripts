...
class RefDbMetrics implements LifecycleListener {
  ...
  void start() {
    ...
    sha1AllRefsMetric =
        metrics.newCallbackMetric(
            "localrefdb/sha1_all_refs_per_project",
            Integer.class,
            new Description("SHA1 of all local refs").setGauge(),
            Field.ofString("repository_name", { it.projectName } as BiConsumer)
...
class CountRefs extends BaseSshCommand {
  ...
  @Argument(index = 0, usage = "Project names", metaVar = "PROJECTS", required = true)
  String[] projects
