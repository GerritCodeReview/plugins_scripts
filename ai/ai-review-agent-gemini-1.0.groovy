...
class AiGeminiReviewProvider implements AiReviewProvider {
    ...
    Set<String> getModels(String apiKey) {
        try {
            httpClient.execute(
                    new HttpGet(GEMINI_API_URL_BASE).with {
    ...
    String review(String apiKey, String model, String prompt) {
        try {
            httpClient.execute(
                    new HttpPost("${GEMINI_API_URL_BASE}/${model}:generateContent").with {
                        setHeader('Content-Type', 'application/json')
                        setHeader('x-goog-api-key', apiKey)
                        entity = new StringEntity(new JsonBuilder([contents: [[parts: [[text: prompt]]]]]).toString(),
            ...
            throw new UncheckedIOException('Failed to call Gemini API', e)
        }
    }
    ...
    private static Set<String> extractModels(String body) {
        ...
        def fetchedModels = json.models?.findAll { model -> model.supportedGenerationMethods?.contains('generateContent') && model.name?.startsWith('models/gemini')
        }?.collect {
