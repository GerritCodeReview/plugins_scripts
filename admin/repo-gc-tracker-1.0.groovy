...
class RepoGCTracker implements LifecycleListener {
  ...
  private static final TMP_IDX_SUFFIX = ".idx_tmp"
  private static final TMP_PACK_SUFFIX = ".pack_tmp"
  ...
  void start() {
    ...
    checkInterval = ConfigUtil.getTimeUnit(pluginConfig.getString("checkInterval", "30s"), 30L, TimeUnit.SECONDS)
    considerStaleAfter = ConfigUtil.getTimeUnit(pluginConfig.getString("considerStaleAfter", "12h"), 43200000L, TimeUnit.MILLISECONDS)
    ...
    logger.atInfo().log("%s checker started for %d projects (checkInterval=%d seconds, staleAfter %d minutes)",
        GC_PID_FILE,
        projectNames.length,
        checkInterval,
  ...
  private void isGCRunningForProject(String projectName) {
    ...
    def hasGc = ((gcPid.exists() && !isFileStale(gcPid)) || hasGCTmpFiles(repoDir))? 1L : 0L
    repoGCTrackerMetrics.gcRunning[projectName] = hasGc
  }

  private boolean hasGCTmpFiles(File repoDir) {
    def packDir = new File(repoDir, GIT_PACK_FOLDER)
    if (!packDir.exists()) {
      return false
    }
    def files = packDir.listFiles()
    def hasIndexTmpFile = files.any { it.name.endsWith(TMP_IDX_SUFFIX) && !isFileStale(it)}
    def hasPackTmpFile = files.any { it.name.endsWith(TMP_PACK_SUFFIX) && !isFileStale(it)}
    return hasIndexTmpFile && hasPackTmpFile
  }
  ...
  private boolean isFileStale(File f) {
    def fileExpiry = f.lastModified() + considerStaleAfter
