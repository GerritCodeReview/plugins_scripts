...
class CertificatesValidityChecker implements LifecycleListener {
  ...
  private static class CheckCertificatesValidityTask implements Runnable {
        ...
        try {
          ...
          for (Certificate cert : certs) {
            if (cert instanceof X509Certificate) {
              ...
              logger.atWarning().log("Certificate type %s, endpoint %s is not a X.509 certificate. Skipping!", cert.getType(), endpoint)
            }
          }
        } catch(e) {
          logger.atSevere()
    ...
    private SSLSocket openConnection(String endpoint) {
      String[] endpointHostAndPort = endpoint.split(":")
      if(endpointHostAndPort.size() != 2) {
        throw new IllegalArgumentException("Wrong endpoint %s format. Correct format: <hostname>:<port>")
      }
      SSLSocketFactory sslSocketFactory = (SSLSocketFactory) SSLSocketFactory.getDefault();
      SSLSocket sslSocket = (SSLSocket) sslSocketFactory.createSocket(endpointHostAndPort[0], endpointHostAndPort[1].toInteger());
