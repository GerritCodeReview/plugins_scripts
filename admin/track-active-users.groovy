...
class TrackAtiveUsersCache extends CacheModule {
    static final NAME = "users_cache"
    @Override
    protected void configure() {
      persist(NAME, Integer, Date)
    }
}
...
class TrackingGroupBackend implements GroupBackend {
  @Inject
  @Named(TrackAtiveUsersCache.NAME) Cache<Integer, Date> trackActiveUsersCache
  ...
  GroupMembership membershipsOf(CurrentUser user) {
    trackActiveUsersCache.put(user.accountId.get(), new Date());
...
class TrackActiveUsersModule extends AbstractModule {
  @Override
  void configure() {
    install(new TrackAtiveUsersCache())
