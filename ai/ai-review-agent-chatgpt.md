# Gerrit AI Review Agent for ChatGPT

Implementation of the Gerrit's AI Code Review Agent API on top of OpenAI's ChatGPT.

[Install](#install-in-gerrit) this plugin and enable the Gerrit AI chat to enjoy
a side-by-side collaboration with ChatGPT on the Change screen.
...
Copy the `ai-review-agent-chatgpt-1.0.groovy` script into your Gerrit site (`$GERRIT_SITE`)
plugins' directory.

```bash
cp ai-review-agent-chatgpt-1.0.groovy "$GERRIT_SITE/plugins/"
```
