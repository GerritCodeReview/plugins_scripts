...
class AutoDisableInactiveUsersExecutor implements Runnable {
  ...
  private static GERRIT_DEFAULT_ADMIN_ACCOUNT_ID = 1000000
  ...
  private void doRun() {
    ...
    for (def accountState : allActiveAccounts) {
      ...
      def accountId = account.id().get()
      if (accountId == GERRIT_DEFAULT_ADMIN_ACCOUNT_ID) {
        // ignore default Gerrit admin account
        continue
      }

      def lastUserActivity = trackActiveUsersCache.getIfPresent(accountId)
      if (!lastUserActivity) {
        accountsToDisable.add(account)
      }
    }

    if (!accountsToDisable.isEmpty()
        && accountsToDisable.size() != allActiveAccounts.size()
        && autoDisableConfig.gracePeriodEnd < now) {
      for (def toDisable : accountsToDisable) {
        disableAccount(toDisable)
      }

      logger.atInfo().log("Automatically disabled %d user(s) after inactivity", accountsToDisable.size())
    }
  }
